...
void MSufSort::Sort(unsigned int ** array, unsigned int count, unsigned int matchLength)
{
	if (count < MIN_LENGTH_FOR_QUICKSORT)
		InsertionSort(array, count, matchLength);
	else
 ...
 void MSufSort::MarkSuffixSorted(unsigned int * suffixIndex)
{
 ...
 void MSufSort::MarkSuffixSorted(unsigned int * suffixIndex, unsigned char symbol)
{
	#ifdef USE_INDUCTION_SORTING
		suffixIndex[1] &= 0xff;
...
bool MSufSort::CompareStrings(unsigned char * stringA, 
...
bool MSufSort::CompareStrings(unsigned int * indexA, unsigned int * indexB)
{
	unsigned int a, b;
...
inline unsigned int MSufSort::GetValue(unsigned int * suffixIndex)
{
...
inline void MSufSort::Swap(unsigned int ** ptrA, unsigned int ** ptrB)
{
...
unsigned int MSufSort::SelectPivot(unsigned int * index1, unsigned int * index2, unsigned int * index3)
{
