...
CCTermOnlineDigestion::CCTermOnlineDigestion()
{
...
inline bool CCTermOnlineDigestion::_IsDiv(size_t p)
{
	return DIVPEP == m_chDat[p];
}

inline bool CCTermOnlineDigestion::_IsEnzyme(size_t p)
{
	return (string::npos != m_strCleave.find(m_chDat[p]) );
}

inline bool CCTermOnlineDigestion::_IsLeftTerm(size_t p)
{
	return DIVPEP == m_chDat[p] || ('M' == m_chDat[p] && DIVPEP == m_chDat[p -1]) ;
}

inline bool CCTermOnlineDigestion::_IsRightTerm(size_t p)
{
	return (DIVPEP == m_chDat[p + 1]);
}

inline bool CCTermOnlineDigestion::_IsLeft(size_t p)//todo 
{
	return ( _IsLeftTerm(p) || _IsEnzyme(p));
}

inline bool CCTermOnlineDigestion::_IsRight(size_t p)
{
	return (_IsRightTerm(p) || _IsEnzyme(p));
}	

inline bool CCTermOnlineDigestion::_IsPep(size_t s, size_t e)
{
...
bool CCTermOnlineDigestion::InitPara(PINDEX_HEAD &pIndexHead,PINDEX_ITEM &pIndexItem, CMetaHandler &pMetaHandler)
{
	...
	{
		proteomics_sdk::CErrInfo err_info("CCTermOnlineDigestion", "InitPara","Can't get enzyme!");
		throw runtime_error(err_info.Get().c_str());
	}
	m_strCleave = enzyme.GetCleaveString();
	m_strNotCleave = enzyme.GetNotCleave();
	//cout << "m_strCleave:" << m_strCleave << endl; 
...
bool CCTermOnlineDigestion::Close()
{
	_DeleteAll();
}

void CCTermOnlineDigestion::_DeleteAll()
{
	try
	{
		if(NULL != m_chDat)
	...
	{
		proteomics_sdk::CErrInfo err_info("CCTermOnlineDigestion", "_DeleteAll");
		throw runtime_error(err_info.Get(e).c_str());
	}
	catch(...)
	{
		proteomics_sdk::CErrInfo err_info("CCTermOnlineDigestion", "_DeleteAll","Unknown Error!");
		throw runtime_error(err_info.Get().c_str());
	}
}
...
bool CCTermOnlineDigestion::GetNextAllPro(size_t tSufType)
{
	size_t clt = clock();
...
bool CCTermOnlineDigestion::GetNextBeg()
{
...
bool CCTermOnlineDigestion::GetNextLCP()
{
	FILE *fl = NULL;
...
bool CCTermOnlineDigestion::GetNextSA()
{
...
void CCTermOnlineDigestion::_InitDigest()
{
	size_t clt = clock();
...
void CCTermOnlineDigestion::_Print(size_t s, size_t e, uchar t,PEP_SQ &strPepSQ)
{
	...
	strPepSQ.cEnd = 0;
	if(_IsLeftTerm(s - 1)) strPepSQ.cEnd |=1;
	if(_IsRightTerm(e)) strPepSQ.cEnd |=2;
}

size_t CCTermOnlineDigestion::GetProID(size_t tp)
{
...
bool CCTermOnlineDigestion::GetPep2Pro(size_t tp, size_t tLen, vector<size_t> & vtProID)
{
	return false;
	vtProID.clear();
	do
...
bool CCTermOnlineDigestion::GetSpecificPep(PEP_SQ &strPepSQ)
{
	bool fl = false;
	for(; m_b < ncp; ++m_b)
	{
...
bool CCTermOnlineDigestion::_GetSemiSpecificPep1(PEP_SQ &strPepSQ)//the situation of specific digestion, the right postion must not the digest position
{
	/*
...
bool CCTermOnlineDigestion::_GetSemiSpecificPep2(PEP_SQ &strPepSQ)//the situation of non-specific digestion, the right postion must be the digest position
{
	/*
	 *  In the suffix system, the first SQ of database begin with '?'
	 */ 
...
bool CCTermOnlineDigestion::GetSemiSpecificPep(PEP_SQ &strPepSQ)
{
...
bool CCTermOnlineDigestion::GetNonSpecificPep(PEP_SQ &strPepSQ)
{
	/*
	 *  In the suffix system, the first SQ of database begin with '?'
	 */ 
...
long long CCTermOnlineDigestion::StaticNonSpecificPep()
{
	long long fn = 0;
	size_t tbg = 0;
...
bool CCTermOnlineDigestion::GetNextBlock()
{
	_DeleteAll();
...
bool CCTermOnlineDigestion::GetNextPep(PEP_SQ &strPepSQ)
{
	switch(m_pIndexItem.nCleaveWay)
	{
		case 0:	
...
bool CCTermOnlineDigestion::SuffixSort(size_t tSufType)
{
	size_t clt = 0;
	FILE *fl = NULL;
	switch(tSufType)
...
void CCTermOnlineDigestion::_GetLCP()
{
	size_t clt = clock();
...
void CCTermOnlineDigestion::_DealSpecial()
{
	size_t clt = clock();
